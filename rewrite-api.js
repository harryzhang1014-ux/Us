...
function getUserKey(suffix: string): string {
  ...
  try {
    ...
    if (raw) {
      ...
      return \`ournest_\${uid}_\${suffix}\`;
    }
  } catch {}
  return \`ournest_anon_\${suffix}\`;
}
export { getUserKey };
...
export async function getPartnerProfile() {
  ...
  if (userId) {
    try {
      const pairContext = await getDbPairContext(userId);
      if (pairContext?.partnerId) {
        const { data: partnerProfile } = await sb.from('profiles').select('id, name, avatar').eq('id', pairContext.partnerId).maybeSingle();
        return {
          id: pairContext.partnerId,
          name: partnerProfile?.name || pairContext.partnerName || 'Partner',
          avatar: partnerProfile?.avatar || '',
        };
      }
    } catch (err) { console.error('getPartnerProfile DB error:', err); }
  }

  const profile = lsGet(getUserKey('profile'), {});
  return { name: profile.partnerName || 'Partner', avatar: '' };
}
...
export async function createPairingCode() {
  ...
  lsSet(\`ournest_pair_\${code}\`, { creatorId: userId, creatorName: userName, partnerId: '', partnerName: '', pairId });
  return { code, pairId };
}
...
export async function joinPairingCode(code: string, partnerName: string) {
  ...
  const pairData = lsGet(\`ournest_pair_\${code.toUpperCase()}\`, null);
...
export async function saveAlbumPhotos(photos: any[]) {
  ...
  if (existing) {
    await sb.from('notes').update({ content }).eq('id', existing.id);
  } else {
    await sb.from('notes').insert({ pair_id: ctx.pairId, author_id: userId, content });
  }
}
...
`;
fs.writeFileSync('src/app/components/api.ts', code);
